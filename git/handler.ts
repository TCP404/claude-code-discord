...
export async function createWorktree(
  ...
  if (
    !existingWorktrees.startsWith("Execution error:") && !existingWorktrees.startsWith("Error:")
  ) {
  ...
  const branchExists = !branchCheckResult.startsWith("Execution error:") &&
  ...
  if (branchExists) {
    ...
    result = await executeGitCommand(
      baseWorkDir,
      `git worktree add ${worktreeDir} -b ${branch} ${actualRef}`,
    );
  }
...
export async function removeWorktree(workDir: string, branch: string): Promise<WorktreeResult> {
  ...
  const result = await executeGitCommand(
    baseWorkDir,
    `git worktree remove ${worktreePathToRemove} --force`,
  );
  ...
  try {
    ...
    if (statusResult && !statusResult.includes("Error") && statusResult.trim()) {
      ...
      const changes = lines.map((line) => {
        ...
        if (
          file.includes("deno.lock") || file.includes(".DS_Store") || file.includes("node_modules/")
        ) {
          return null;
        }
