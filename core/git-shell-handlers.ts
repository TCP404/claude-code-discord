    ...
    ["worktree", {
      execute: async (ctx: InteractionContext) => {
        ...
        try {
          ...
          const isError = result.result.startsWith("Execution error:") ||
            result.result.startsWith("Error:") || result.result.includes("fatal:");

          if (!isError || result.isExisting === true) {
            await ctx.editReply({
              embeds: [{
                color: 0xffff00,
                title: result.isExisting === true
                  ? "Worktree Detected - Starting Bot..."
                  : "Worktree Created Successfully - Starting Bot...",
                fields: [
            ...
            try {
                  ...
                  fields: [
                    ...
                    { name: "Result", value: `\`\`\`\n${result.result}\n\`\`\``, inline: false },
                    {
                      ...
                      value: result.isExisting === true
                        ? "✅ Bot process started in existing Worktree"
                        : "✅ New Bot process started",
                      inline: false,
                    },
                  ],
                  timestamp: true,
              ...
              await ctx.editReply({
                embeds: [{
                  color: 0xff9900,
                  title: result.isExisting === true
                    ? "Worktree Detected - Bot Start Failed"
                    : "Worktree Created Successfully - Bot Start Failed",
                  fields: [
                    { name: "Branch", value: branch, inline: true },
                    { name: "Path", value: result.fullPath, inline: false },
                    { name: "Result", value: `\`\`\`\n${result.result}\n\`\`\``, inline: false },
      ...
      execute: async (ctx: InteractionContext) => {
        ...
        await ctx.editReply({
          embeds: [{
            ...
            description: status.totalBots === 0
              ? "No worktree bots running."
              : `${status.totalBots} worktree bot(s) running:`,
                ...
                await ctx.editReply({
                  embeds: [{
                    ...
                    fields: [
                      {
                        name: "Process ID",
                        value: executionResult.processId.toString(),
                        inline: true,
                      },
                      {
                        name: "Status",
                        value: "Long-running process... (will update when complete)",
                        inline: false,
                      },
                    ],
                    timestamp: true,
            ...
            await ctx.editReply({
              embeds: [{
                ...
                fields: [
                  ...
                  {
                    name: "Result",
                    value: "✅ Input sent. New output will be displayed below if available.",
                    inline: false,
                  },
                ],
                timestamp: true,
            ...
            await ctx.editReply({
              embeds: [{
                ...
                fields: [
                  ...
                  {
                    name: "Result",
                    value: "❌ Process not found. The process may have terminated.",
                    inline: false,
                  },
                ],
                timestamp: true,
          ...
          await ctx.editReply({
            embeds: [{
              ...
              fields: [
                ...
                {
                  name: "Result",
                  value: result.success ? "Process stopped" : "Process not found",
                  inline: false,
                },
              ],
              timestamp: true,
...
export function createUtilityCommandHandlers(
  ...
  const { git: gitHandlers, shell: shellHandlers, utils: utilsHandlers, help: helpHandlers } =
    handlers;
        ...
        await ctx.editReply({
          embeds: [{
            ...
            fields: [
              ...
              {
                name: "Mentions",
                value: botSettings.mentionEnabled
                  ? `Enabled (<@${botSettings.mentionUserId}>)`
                  : "Disabled",
                inline: true,
              },
            ],
