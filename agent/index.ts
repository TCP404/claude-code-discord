...
import { type AgentSessionData, getAgentSessionsManager } from "../util/persistence.ts";
...
export const PREDEFINED_AGENTS: Record<string, AgentConfig> = {
  ...
  "opus-coder": {
    ...
    description:
      "Most powerful coding agent using Claude Opus - for complex codebases and long-running tasks",
    model: "opus",
    systemPrompt:
  ...
  "opus-architect": {
    ...
    description:
      "Senior system architect using Claude Opus - for complex design decisions and large-scale systems",
    model: "opus",
    systemPrompt:
...
async function listAgents(ctx: any) {
  const agentList = Object.entries(PREDEFINED_AGENTS).map(([key, agent]) => {
    const riskEmoji = agent.riskLevel === "high"
      ? "🔴"
...
async function startAgentSession(ctx: any, agentName: string) {
  ...
  const riskColor = agent.riskLevel === "high"
    ? 0xff6600
    : agent.riskLevel === "medium"
    ? 0xffaa00
    : 0x00ff00;
  ...
  await ctx.editReply({
    embeds: [{
      ...
      fields: [
        ...
        {
          name: "Usage",
          value: "Use `/agent action:chat message:[your message]` to chat with this agent",
          inline: false,
        },
      ],
  ...
  if (!activeAgentName) {
    await ctx.editReply({
      embeds: [{
        ...
        description:
          "No agent session active. Use `/agent action:start agent_name:[name]` to start one.",
        timestamp: true,
      }],
    });
    return;
    ...
    embeds: [{
      ...
      fields: [
        ...
        { name: "Risk Level", value: agent.riskLevel, inline: true },
        {
          ...
          value: `\`${message.substring(0, 200)}${message.length > 200 ? "..." : ""}\``,
          inline: false,
        },
      ],
      timestamp: true,
    ...
    await ctx.editReply({
      embeds: [{
        ...
        fields: [
          ...
          {
            name: "Session",
            value: result.sessionId ? `\`${result.sessionId.substring(0, 12)}\`` : "N/A",
            inline: true,
          },
        ],
    ...
    embeds: [{
      ...
      fields: [
        ...
        { name: "Capabilities", value: agent.capabilities.join(", "), inline: false },
        {
          ...
          value: `\`${agent.systemPrompt.substring(0, 200)}...\``,
          inline: false,
        },
      ],
      timestamp: true,
  ...
  await ctx.editReply({
    embeds: [{
      ...
      fields: [
        {
          name: "Previous Agent",
          value: previousAgent ? PREDEFINED_AGENTS[previousAgent]?.name || "None" : "None",
          inline: true,
        },
...
async function endAgentSession(ctx: any) {
  ...
  await ctx.editReply({
    embeds: [{
      color: 0x00ff00,
      title: "✅ Session Ended",
      description: `Agent session with ${
        PREDEFINED_AGENTS[activeAgent]?.name || activeAgent
      } has been ended.`,
